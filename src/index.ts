 ...
 */
import { Identity, Fin } from "../node_modules/hadouken-js-adapter/out/types/src/main";
...
window.fdc3 = {
    ...
    Agent: {
        create: async (name: string) => {
            ...
            await fin.InterApplicationBus.subscribe({ uuid: '*' }, 'FDC3P2P/get-all-agents', (_: any, source: Identity) => fin.InterApplicationBus.send(source, 'FDC3P2P/add-agent', {channel, name}) )
            const provider = await fin.InterApplicationBus.Channel.create(channel);
            const agent = new FDC3Agent(name, provider);
            agents.set(name, agent)
            setTimeout(() => fin.InterApplicationBus.publish('FDC3P2P/add-agent', {channel, name}), 100) // give time to add connection listener
            return agent
        },
        connect: async (name: string, payload?: any) => {
            ...
            const client = await fin.InterApplicationBus.Channel.connect('FDC3P2P' + name, payload)
    ...
    async raiseIntent(intent: string, context: object, target?: string | undefined): Promise<IntentResolution> {
        ...
        return new Promise((resolve, reject) => {
            ...
            for (const client of connectionsToAgents.values()) {
                const p = client.raiseIntent(intent, context, target).then(v => {
                    resolve(v);
                }).catch(e => {
    ...
    addContextListener(handler: (context: object) => void): Listener {
        ...
        const unsubscribe = () => {
            globalContextListeners = globalContextListeners.filter(x => x !== handler);
            unsubscribes.forEach(x => x());
        }
        return {
            unsubscribe,
...
async function init() {
    await fin.InterApplicationBus.subscribe({uuid: '*'}, 'FDC3P2P/add-agent', async (info: any, source: Identity) => {
        if(!agents.has(info.name)) {
            const channel = await fin.InterApplicationBus.Channel.connect(info.channel);
            ...
            globalIntentListeners.forEach(([i, f]) => client.addIntentListener(i, f));
        }
    })
    await fin.InterApplicationBus.subscribe({uuid: '*'}, 'FDC3P2P/globalbroadcast', (context: any) => {
...
class FDC3Client implements DesktopAgent {
    ...
    constructor(public name: string, private client: ChannelClient) {
        ...
        this.client.register('joined-channel', async ({id, iabTopic}: {iabTopic: string, id: string}) => {
            if (id !== 'default') subscribed = name;
            this.iabTopic = iabTopic;
            await fin.InterApplicationBus.subscribe({uuid: '*'}, iabTopic, this._handleContext);
        })
        this.client.register('left-channel', async ({ iabTopic }: { iabTopic: string }) => {
            if (subscribed === this.name) subscribed = null;
            this.iabTopic = null;
            await fin.InterApplicationBus.unsubscribe({uuid: '*'}, iabTopic, this._handleContext)
        })
    }

    findIntent = (intent: string, context?: object | undefined): Promise<AppIntent> => {
    ...
    raiseIntent = (intent: string, context: object, target?: string | undefined): Promise<IntentResolution> => {
        return this.client.dispatch('raise-intent', {intent, context, target});
    }
    addIntentListener = (intent: string, handler: (context: object) => void): Listener => {
        throw new Error("Method not implemented.");
    }
    addContextListener = (handler: (context: object) => void): Listener => {
    ...
    async getAllChannels() {
        ...
        const decorateInfo = (info: any) => Object.assign(info, {join: (identity = me) => this.joinChannel(info.id, me)})
        return info.map(decorateInfo)
    }
    private async joinChannel(channelId: string, identity: Identity= me) {
        await this.client.dispatch('join-channel', {channelId, identity})
    }
...
class FDC3Agent {
    ...
    constructor(public name: string, public provider: ChannelProvider) {
        ...
        provider.register('join-channel', async ({channelId, identity, previousChannel}: {channelId: string, previousChannel: string, identity: Identity}) => {
            if (!this.channels.has(channelId)) {
                throw new Error('Could not locate requested channel');
            }
            if (previousChannel && this.channels.has(previousChannel)) {
                const prev: any = this.channels.get(previousChannel);
                await provider.dispatch(identity, 'left-channel', prev)
                prev.members = prev.members.filter((x: Identity) => x.uuid !== identity.uuid && x.name !== identity.name);
            }
            const joining = this.channels.get(channelId);
            await provider.dispatch(identity, 'joined-channel', joining);
            await fin.InterApplicationBus.send(identity, joining.iabTopic, joining.context);
        })
        provider.register('get-channel-info', () => this.channels)
        provider.setDefaultAction(() => {
            throw new Error('FDC3 Agent has not implemented this method')
        })
    }
    registerOpenHandler = async (handler: (name: string, context: any) => void) => {
        this.provider.register('open', ({ context, name }: {
            name: string;
            context: any;
        }) => handler(name, context));
    }
    registerIntentResolver = async (handler: (intent: string, context: any, target?: string, identity?: Identity) => Promise<IntentResolution>) => {
         this.provider.register('raise-intent', ({intent, context, target}: any, sender) => handler(intent, context, target, sender))
    }
